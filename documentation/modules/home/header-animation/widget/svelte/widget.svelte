<script>
    ...
    const onMouseMove = event => {
        ...
        if (xDirection !== 'same') {
            ...
            target.style.left = `${mouseCurrentX}px`;
        }

        if (yDirection !== 'same') {
            mouseCurrentY += yDirection === 'top' ? -5 : 5;
            mouseCurrentY = mouseCurrentY > 70 ? 70 : mouseCurrentY < -70 ? -70 : mouseCurrentY;
            target.style.top = `${mouseCurrentY}px`;
        }

        target.style.transform = `rotate(${quadrants[index].method()}deg)`;
