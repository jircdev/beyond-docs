...
The modules are created and managed through the module.json configuration files. These have the alternative of being created manually or through the Workspace interface.

<Subtitle>Creation from the Workspace</Subtitle>

To be able to create a module, first you must have created a project before, and this needs to be active in the Workspace.

The option to create projects is a feature available when a project is active on the Workspace. In order to see them, it is necessary to open the project by accessing its detail board.
