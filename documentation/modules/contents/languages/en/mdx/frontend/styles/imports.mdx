...
<DocHeader pretitle="Widgets" title="Importing style files" />

<Title tag="h4" text="node_modules" />

If it is necessary to include a stylesheet from a library installed in node_modules,
it can be imported non-relatively, from the module file where it is required, by using the `~`
character followed by the package path in the `node_modules` folder.
...
<Title tag="h4" text="Importing styles from a bundle" />
