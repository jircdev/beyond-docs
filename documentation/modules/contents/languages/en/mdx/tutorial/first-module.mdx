...
As you can see, the project has two modules created: the `home` module, which corresponds to the base URL of the project, and the `start-backend` module, which will be discussed later.

The structure of the `home` module is as follows:

-   `widget`: folder where the code of the page component is located.
-   `ts`:
...
If you've worked with React, what comes next is the part you probably know best, with a few minor differences. Next, we will edit our `index.tsx` file to create a login form and the basic functionality to update the status.
...
You can verify this if you open the code parser, you may notice a `<app-home/>` tag that encapsulates the content of the module created.
...
> The code bundled in the development environment does not have optimizations to be able to do.
...
> In addition, <BeyondJS/> can also generate the module type declarations. You can read
> about this in <Link href="/docs/modules/bundles">the bundles section</Link>.
